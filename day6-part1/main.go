...
var (
	reCmd = regexp.MustCompile(`^(.*?)\s(\d+),(\d+)\sthrough\s(\d+),(\d+)$`)
)
...
func newCommand(in string) *command {
	res := reCmd.FindAllStringSubmatch(in, -1)
...
func (g *grid) addLight(x, y int) {
	l := &light{x: x, y: y, on: false}
...
func (g *grid) run(c *command) {
	for _, l := range g.lights {
		...
		case "turn off":
			l.turnOff(c.gr)
		default:
			log.Fatalf("don't know anything about %q", c.action)
		}
	}
}
...
func newGridRange(x1, y1, x2, y2 int) *gridRange {
	return &gridRange{
		startX: x1,
		startY: y1,
		endX:   x2,
...
func (gr *gridRange) containsPoint(x, y int) bool {
	return (x >= gr.startX && x <= gr.endX) &&
		(y >= gr.startY && y <= gr.endY)
}
...
func (l *light) toggle(gr *gridRange) {
	if l.inRange(gr) {
		l.on = !l.on
	}
}

func (l *light) turnOn(gr *gridRange) {
	if l.inRange(gr) {
		l.on = true
	}
}

func (l *light) turnOff(gr *gridRange) {
	if l.inRange(gr) {
		l.on = false
	}
}

func (l *light) inRange(gr *gridRange) bool {
	cp := gr.containsPoint(l.x, l.y)
	return cp
}

var (
	testCmds = `toggle 0,0 through 999,0`
	cmds     = `turn off 660,55 through 986,197
